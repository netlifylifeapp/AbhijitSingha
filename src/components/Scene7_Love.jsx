...
const timeline = [
  ...
  },
  {
    ...
    detail: 'I cared too much. And in doing so, I gave her a reason to care less.'
  },
  {
    phase: 'the after',
    icon: '\u2014',
    ...
    >
      <div className="flex items-start gap-5">
        ...
        <div className="flex-1 pb-8">
          <div className="font-mono text-[9px] tracking-[0.3em] mb-2 uppercase transition-colors duration-300" style={{ color: open ? item.color : 'rgba(100,130,180,0.4)' }}>
            {item.phase}
          </div>
          <p className="font-serif text-[14px] md:text-[15px] text-[rgba(190,210,250,0.65)] leading-relaxed italic mb-2">{item.text}</p>
          <AnimatePresence>
            {open && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.4 }} className="overflow-hidden">
                <div className="mt-3 pl-4 py-3 border-l" style={{ borderColor: item.color }}>
                  <p className="font-sans text-[12px] text-[rgba(150,180,230,0.55)] leading-relaxed">{item.detail}</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          <div className="font-mono text-[8px] text-[rgba(80,100,140,0.3)] mt-2">{open ? '— click to collapse' : '— click to expand'}</div>
        </div>
      </div>
    </motion.div>
  )
...
export default function Scene7_Love() {
  ...
  return (
    <section ref={ref} className="scene-section relative z-10 py-20">
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 70% 80% at 50% 50%, rgba(8,3,18,0.98) 0%, transparent 100%)' }} />
      <div className="absolute pointer-events-none" style={{ left: '15%', top: '25%', width: '400px', height: '400px', background: 'radial-gradient(circle, rgba(255,107,157,0.04) 0%, transparent 70%)', borderRadius: '50%' }} />
      <div className="relative z-10 max-w-4xl mx-auto px-6 md:px-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 1 }} className="text-center mb-4">
          <div className="font-mono text-[9px] tracking-[0.5em] text-[rgba(255,107,157,0.35)] uppercase mb-4">chapter seven</div>
          <h2 className="font-serif text-3xl md:text-5xl text-[rgba(200,215,255,0.65)] italic mb-3">the story that stayed</h2>
        </motion.div>
        <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ delay: 0.3, duration: 1 }} className="mb-12">
          {showPulse && <HeartbeatLine isInView={isInView} />}
        </motion.div>
        <div className="relative">
          {timeline.map((item, i) => <TimelineCard key={i} item={item} index={i} isInView={isInView} />)}
        </div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 2, duration: 1.5 }} className="mt-10 text-center">
          <div className="w-8 h-px bg-[rgba(255,107,157,0.3)] mx-auto mb-8" />
          <blockquote className="font-serif text-xl md:text-3xl leading-relaxed max-w-2xl mx-auto" style={{ color: 'rgba(220,230,255,0.55)', textShadow: '0 0 40px rgba(255,107,157,0.1)' }}>
            "Some people break hearts through hatred. He broke hers while trying too hard to protect his own."
          </blockquote>
